...
import {
  ...
  ArrowUpRight, X, CalendarDays, DollarSign, ChevronRight,
...
const STATUS_OPT = [
  { value: 'nao_iniciado', label: 'Não iniciado', color: '#50505E' },
...
export default function DashboardPage() {
  ...
  const projetosRecentes = [...projetos].sort((a, b) => b.criadoEm.localeCompare(a.criadoEm)).slice(0, 5)
  const txRecentes       = [...transacoes].sort((a, b) => b.data.localeCompare(a.data)).slice(0, 5)

  const statusCount = STATUS_OPT.map(s => ({
    ...s,
    count: projetos.filter(p => p.status === s.value).length,
  })).filter(s => s.count > 0)
  ...
  return (
    <div className="min-h-full" style={{ background: 'var(--bg)' }}>
      <div className="max-w-5xl mx-auto px-5 lg:px-8 py-7 lg:py-9">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          ...
          <KpiCard
            ...
            sub={`${formatMoeda(saidas)} em saídas`}
        ...
        {statusCount.length > 0 && (
          <div
            className="rounded-2xl px-5 py-4 mb-6 anim-up d-2"
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/projetos" className="flex items-center gap-1 text-[11px] font-semibold transition-opacity hover:opacity-70" style={{ color: 'var(--accent)' }}>
        ...
        {/* ── Ações Rápidas ── */}
        <div className="mb-6 anim-up d-2">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] mb-3" style={{ color: 'var(--text3)' }}>
            Ações rápidas
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Novo Cliente',    icon: <UserPlus size={16} />,     href: '/clientes',  cor: '#60A5FA' },
              { label: 'Novo Projeto',    icon: <FolderKanban size={16} />, href: '/projetos',  cor: '#F5C518' },
              { label: 'Nova Transação',  icon: <Plus size={16} />,         href: '/financeiro',cor: '#22C55E' },
              { label: 'Ver Financeiro',  icon: <Wallet size={16} />,       href: '/financeiro',cor: '#FB923C' },
            ].map(a => (
              ...
              >
                ...
                <span className="text-sm font-medium" style={{ color: 'var(--text2)' }}>{a.label}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-4">
          ...
          {/* Projetos recentes */}
          <section
            className="lg:col-span-3 rounded-2xl overflow-hidden anim-up d-3"
            style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
          >
                      ...
                      <li key={p.id}>
                        ...
                        >
                          <span
                            ...
                            style={{ background: st?.color, boxShadow: `0 0 5px ${st?.color}55` }}
                          />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate" style={{ color: 'var(--text)' }}>{p.nome}</p>
                            <p className="text-xs truncate mt-0.5" style={{ color: 'var(--text2)' }}>
                              {p.clienteNome || 'Sem cliente'} · <span style={{ color: st?.color }}>{st?.label}</span>
                            </p>
                          </div>
                          <div className="shrink-0 text-right">
                            <p className="text-sm font-semibold font-mono" style={{ color: 'var(--text)' }}>{formatMoeda(p.valor)}</p>
                            {p.prazo && (
                          ...
                          <ChevronRight size={13} className="opacity-0 group-hover:opacity-30 transition-opacity shrink-0" style={{ color: 'var(--text3)' }} />
                        </button>
                      </li>
                    )
                  })}
          ...
          {/* Transações recentes */}
          <section
            className="lg:col-span-2 rounded-2xl overflow-hidden anim-up d-4"
            style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
          >
                ...
                <ul>
                  {txRecentes.map((t, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium truncate" style={{ color: 'var(--text)' }}>{t.descricao}</p>
      ...
      {projetoSel && (
        ...
        >
          <div
            className="w-full max-w-lg rounded-2xl overflow-hidden anim-scale"
          ...
          >
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold tracking-[0.14em] uppercase mb-1" style={{ color: 'var(--text3)' }}>
                  {projetoSel.clienteNome || 'Sem cliente'}
                </p>
                <h2 className="font-semibold text-lg leading-snug" style={{ color: 'var(--text)' }}>{projetoSel.nome}</h2>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-px" style={{ background: 'var(--border)', borderBottom: '1px solid var(--border)' }}>
              <div className="flex items-center gap-3 px-6 py-4" style={{ background: 'var(--surface2)' }}>
                <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'var(--surface3)' }}>
              ...
              <div className="flex items-center gap-3 px-6 py-4" style={{ background: 'var(--surface2)' }}>
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold mt-0.5">{projetoSel.prazo ? formatData(projetoSel.prazo) : '—'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-5 space-y-5">
              {projetoSel.descricao && (
                <div>
                  <p className="text-[10px] font-semibold tracking-[0.12em] uppercase mb-2" style={{ color: 'var(--text2)' }}>
                    Descrição
                  </p>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--text)' }}>{projetoSel.descricao}</p>
                </div>
              )}

              <div>
                <p className="text-[10px] font-semibold tracking-[0.12em] uppercase mb-2.5" style={{ color: 'var(--text2)' }}>
                  Status do projeto
                </p>
...
function KpiCard({ label, value, sub, icon, href, delay, accent, valueColor }: {
  label: string; value: string; sub: string; icon: React.ReactNode
  href: string; delay: string; accent?: boolean; valueColor?: string
}) {
  return (
    <Link
      href={href}
      className={`anim-up ${delay} rounded-2xl p-5 flex flex-col gap-3 group transition-all duration-150`}
      style={{
    ...
    >
      <div className="flex items-center justify-between">
        <span
          ...
          style={{
            background: accent ? 'var(--accent-low)' : 'var(--surface2)',
            color:      accent ? 'var(--accent)' : 'var(--text2)',
          }}
        >
          {icon}
        </span>
        <ArrowUpRight size={13} className="opacity-0 group-hover:opacity-40 transition-opacity" style={{ color: 'var(--text3)' }} />
      </div>
      <div>
        <p
        ...
        <p className="text-[10px] font-semibold uppercase tracking-[0.12em] mt-2" style={{ color: 'var(--text3)' }}>
          {label}
        </p>
